package org.coursera.scala.functional.week4.assignment.patmat
...
object Huffman {
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.map { case (char, freq) => Leaf(char, freq) }.sortBy(leaf => leaf.weight)
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case first :: second :: xs => (makeCodeTree(first, second) :: xs).sortBy(leaf => weight(leaf))
    case _ => trees
  }
  ...
  /**
    ...
    */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find { case (x, _) => x == char }.get._2
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(b: Bit)(charCode: (Char, List[Bit])): (Char, List[Bit]) = (charCode._1, b :: charCode._2)

    //left tree prepend with 0, right prepend with 1
