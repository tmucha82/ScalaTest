package org.coursera.scala.parallel.week3.assignment.kmeans.fun

import org.coursera.scala.parallel.week3.assignment.kmeans.{Point, KMeans}
...
class IndexedColorFilter(initialImage: Img,
  ...
  private def indexedImage(img: Img, means: GenSeq[Point]): Img = {
  ...
  override def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    convStrategy match {
      case ConvergedAfterNSteps(n) =>
        steps >= n
      case ConvergedAfterMeansAreStill(x) =>
        super.converged(x)(oldMeans, newMeans)
